...
public class BarMakerBlock extends BlockContainer {
	protected Random rand = new Random();

	static boolean isActiveSound;
	
	public BarMakerBlock(String BarMaker) {
		super(Material.rock);
		setBlockName(Ref.MODID + "_" + BarMaker);
	...
	public void breakBlock(World par1World, int x, int y, int z, Block blockID,
		...
		if (te != null && te instanceof BarMakerLogic) {
			BarMakerLogic logic = (BarMakerLogic) te;
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (logic instanceof BarMakerLogic) {
			BarMakerLogic direction = (BarMakerLogic) logic;
		...
		if (logic instanceof BarMakerLogic) {
			BarMakerLogic inv = (BarMakerLogic) logic;
	...
	public static boolean isActive(IBlockAccess world, int x, int y, int z) {
		TileEntity logic = world.getTileEntity(x, y, z);
		if (logic instanceof BarMakerLogic) {
				return ((BarMakerLogic) logic).getActive();
		}
		return false;
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (te instanceof BarMakerLogic) {
			return ((BarMakerLogic) te).getActive() ? 15 : 0;
		}
	...
	public void randomDisplayTick(World world, int x, int y, int z,
			Random par5Random) {
		BarMakerLogic logic = (BarMakerLogic) world.getTileEntity(x, y, z);
		if (logic.getActive()) {
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
		TileEntity logic = world.getTileEntity(x, y, z);
		int direction = (logic instanceof BarMakerLogic) ? ((BarMakerLogic) logic)
				.getRenderDirection() : 0;
		int meta = world.getBlockMetadata(x, y, z) % 8;

		if (meta == 0) {
			if (side == direction) {
				if (((BarMakerLogic) logic).getActive())
					return icons[1];
				else
					return icons[0];
			} else if (side > 1) {
	...
	public String[] getTextureNames() {
		String[] textureNames = { "BarMakerFrunt", "BarMakerLit",
				"BarMakerSide", "BarMakerTop" };
