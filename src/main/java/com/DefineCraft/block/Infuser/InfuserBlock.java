...
public class InfuserBlock extends BlockContainer {
	...
	public InfuserBlock(String Infuser) {
		super(Material.rock);
		setBlockName(Ref.MODID + "_" + Infuser);
	...
	public void breakBlock(World par1World, int x, int y, int z, Block blockID,
		...
		if (te != null && te instanceof InfuserLogic) {
			InfuserLogic logic = (InfuserLogic) te;
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (logic instanceof InfuserLogic) {
			InfuserLogic direction = (InfuserLogic) logic;
		...
		if (logic instanceof InfuserLogic) {
			InfuserLogic inv = (InfuserLogic) logic;
	...
	public static boolean isActive(IBlockAccess world, int x, int y, int z) {
		TileEntity logic = world.getTileEntity(x, y, z);
		if (logic instanceof InfuserLogic) {
				return ((InfuserLogic) logic).getActive();
		}
		return false;
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (te instanceof InfuserLogic) {
			return ((InfuserLogic) te).getActive() ? 15 : 0;
		}
	...
	public void randomDisplayTick(World world, int x, int y, int z,
			Random par5Random) {
		InfuserLogic logic = (InfuserLogic) world.getTileEntity(x, y, z);
		if (logic.getActive()) {
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
		TileEntity logic = world.getTileEntity(x, y, z);
		int direction = (logic instanceof InfuserLogic) ? ((InfuserLogic) logic)
				.getRenderDirection() : 0;
		int meta = world.getBlockMetadata(x, y, z) % 8;

		if (meta == 0) {
			if (side == direction) {
				if (((InfuserLogic) logic).getActive())
					return icons[1];
				else
					return icons[0];
			} else if (side > 1) {
	...
	public String[] getTextureNames() {
		String[] textureNames = { "InfuserFrunt", "InfuserLit",
				"InfuserSide", "InfuserTop" };
