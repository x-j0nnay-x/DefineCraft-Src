package com.DefineCraft.block.Grinder;
...
public class MaterBlock extends BlockContainer {
	protected Random rand = new Random();

	static boolean isActiveSound;
	
	public MaterBlock(String Grinder) {
		super(Material.rock);
		setBlockName(Ref.MODID + "_" + Grinder);
	...
	public void breakBlock(World par1World, int x, int y, int z, Block blockID,
		...
		if (te != null && te instanceof GrinderLogic) {
			GrinderLogic logic = (GrinderLogic) te;
	...
	public void onBlockPlacedBy(World world, int x, int y, int z,
		...
		if (logic instanceof GrinderLogic) {
			GrinderLogic direction = (GrinderLogic) logic;
		...
		if (logic instanceof GrinderLogic) {
			GrinderLogic inv = (GrinderLogic) logic;
	...
	public static boolean isActive(IBlockAccess world, int x, int y, int z) {
		TileEntity logic = world.getTileEntity(x, y, z);
		if (logic instanceof GrinderLogic) {
				return ((GrinderLogic) logic).getActive();
		}
		return false;
	}
	...
	public int getLightValue(IBlockAccess world, int x, int y, int z) {
		...
		if (te instanceof GrinderLogic) {
			return ((GrinderLogic) te).getActive() ? 15 : 0;
		}
	...
	public void randomDisplayTick(World world, int x, int y, int z,
			Random par5Random) {
		GrinderLogic logic = (GrinderLogic) world.getTileEntity(x, y, z);
		if (logic.getActive()) {
	...
	public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
		TileEntity logic = world.getTileEntity(x, y, z);
		int direction = (logic instanceof GrinderLogic) ? ((GrinderLogic) logic)
				.getRenderDirection() : 0;
		int meta = world.getBlockMetadata(x, y, z) % 8;

		if (meta == 0) {
			if (side == direction) {
				if (((GrinderLogic) logic).getActive())
					return icons[1];
				else
					return icons[0];
			} else if (side > 1) {
	...
	public String[] getTextureNames() {
		String[] textureNames = { "GrinderFace", "GrinderLit",
				"GrinderSide", "GrinderTop" };
